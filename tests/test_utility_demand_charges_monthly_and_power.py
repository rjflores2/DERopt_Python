...
def test_flat_demand_creates_month_peaks_only_for_months_in_run_and_uses_month_rates():
    ...
    assert (2024, 0, "electricity_load__x", "_default") in flat_keys
    assert (2024, 1, "electricity_load__x", "_default") in flat_keys
    assert (2024, 2, "electricity_load__x", "_default") not in flat_keys

    # Set imports so Jan peak=5 kW and Feb peak=7 kW
    m.utility.grid_import["electricity_load__x", "_default", 0].value = 5.0  # kWh in 1 hour => 5 kW
    m.utility.grid_import["electricity_load__x", "_default", 1].value = 7.0  # => 7 kW
    m.utility.P_flat[2024, 0, "electricity_load__x", "_default"].value = 5.0
    m.utility.P_flat[2024, 1, "electricity_load__x", "_default"].value = 7.0
...
def test_flat_demand_multiyear_creates_separate_year_month_peaks():
    ...
    flat_keys = set(m.utility.P_flat.keys())
    assert (2024, 0, "electricity_load__x", "_default") in flat_keys
    assert (2025, 0, "electricity_load__x", "_default") in flat_keys

    # Set imports so the peaks differ by year.
    m.utility.grid_import["electricity_load__x", "_default", 0].value = 5.0
    m.utility.grid_import["electricity_load__x", "_default", 1].value = 7.0
    m.utility.P_flat[2024, 0, "electricity_load__x", "_default"].value = 5.0
    m.utility.P_flat[2025, 0, "electricity_load__x", "_default"].value = 7.0
...
def test_tou_demand_creates_month_tier_peaks_only_when_tier_occurs_in_month():
    ...
    assert (2024, 0, 1, "electricity_load__x", "_default") in tou_keys
    assert (2024, 1, 2, "electricity_load__x", "_default") in tou_keys
    assert (2024, 0, 2, "electricity_load__x", "_default") not in tou_keys
    assert (2024, 1, 1, "electricity_load__x", "_default") not in tou_keys
...
def test_tou_demand_charge_cost_sums_month_tier_peaks():
    ...
    m.utility.P_tou[2024, 0, 1, "electricity_load__x", "_default"].value = 5.0
    m.utility.P_tou[2024, 1, 2, "electricity_load__x", "_default"].value = 7.0
...
def test_tou_demand_multiyear_creates_separate_year_month_peaks():
    ...
    m = build_model(data, technology_parameters={}, financials={})
    tou_keys = set(m.utility.P_tou.keys())
    assert (2024, 0, 0, "electricity_load__x", "_default") in tou_keys
    assert (2025, 0, 0, "electricity_load__x", "_default") in tou_keys

    m.utility.P_tou[2024, 0, 0, "electricity_load__x", "_default"].value = 5.0
    m.utility.P_tou[2025, 0, 0, "electricity_load__x", "_default"].value = 7.0
...
def test_subhourly_demand_charge_uses_power_conversion():
    ...
    data.utility_rate_by_node = {"electricity_load__x": data.utility_rate}
    m = build_model(data, technology_parameters={}, financials={})
    m.utility.grid_import["electricity_load__x", "_default", 0].value = 25.0
    m.utility.P_flat[2024, 0, "electricity_load__x", "_default"].value = 0.0
    power = pyo.value(m.utility.grid_import_power_kw["electricity_load__x", "_default", 0])
