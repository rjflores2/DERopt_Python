...
from data_loading.loaders.energy_load import load_energy_demand


def test_load_energy_demand_smoke(tmp_path: Path):
...
def test_load_energy_demand_fallback_detects_kw_column(tmp_path: Path):
...
def test_load_energy_demand_fallback_detects_kwh_column(tmp_path: Path):
...
def test_load_energy_demand_loads_multiple_columns(tmp_path: Path):
...
def test_load_energy_demand_duplicate_headers(tmp_path: Path):
...
def test_load_energy_demand_excel_serial(tmp_path: Path):
...
def test_load_energy_demand_matlab_serial(tmp_path: Path):
...
def test_load_energy_demand_auto_detects_excel_serial(tmp_path: Path):
...
def test_load_energy_demand_xlsx_smoke(tmp_path: Path):
...
def test_load_energy_demand_xlsx_sheet_name(tmp_path: Path):
...
def test_time_conditioning_resamples_to_regular_grid(tmp_path: Path):
    ...
    assert data.static["time_step_hours"] == 1.0
    demands = data.timeseries["electricity_demand"]
    assert demands[0] == 10.0
    assert demands[3] == 14.0
...
def test_igiugig_xlsx_case_loads():
    ...
    data = load_energy_demand(cfg.energy_load)
    assert len(data.indices["time"]) > 0
    assert "electricity_demand" in data.timeseries
